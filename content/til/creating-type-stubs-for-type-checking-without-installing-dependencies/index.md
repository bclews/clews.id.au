...
Python's import system doesn't distinguish between packages installed via pip and packages manually created in `site-packages`. Both are treated identically.

```bash
# These are equivalent to Python's import system:
pip install wet-dry-gap                    # Real package
mkdir -p .venv/lib/python3.X/site-packages/wet_dry_gap  # Fake stub
```
...
STUB_DIR=".venv/lib/python$(python3 -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")')/site-packages/wet_dry_gap"
mkdir -p "$STUB_DIR"

# Create minimal __init__.py with type signatures only
cat > "$STUB_DIR/__init__.py" << 'EOF'
"""Type stubs for wet-dry-gap package (type checking only)."""
import pandas as pd

def process_netcdf_tidal_data(
    netcdf_file: str,
...
1. **Function signatures with types** - Include all parameters with type annotations and return types
2. **Ellipsis (`...`) for implementation** - Standard Python way to indicate stub/placeholder
3. **Minimal imports** - Only import what's needed for type hints (e.g., `pandas` for `pd.DataFrame`)
4. **No actual logic** - The code can't run, but Pyright can validate against it

### What Pyright Sees

When analyzing your code:

```python
from wet_dry_gap import process_netcdf_tidal_data

tidal_df = process_netcdf_tidal_data(netcdf_file="foo.nc")
```

Pyright:
1. Looks in `site-packages/wet_dry_gap/__init__.py`
...
Both approaches work, but creating a minimal Python module is simpler and doesn't require understanding the `.pyi` stub file format.

## Real-World Use Case

In our PyWPS project, `wet-dry-gap` has dependencies on:
- NetCDF4 (C library bindings)
- XBeach simulation tools
- Matplotlib, NumPy, Pandas
- Various scientific computing libraries

For type checking `beachlandinganalysis.py`, we only need to know:
